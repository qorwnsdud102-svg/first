...
- [[Claude-Skill]] — 본 가이드라인은 user-level 스킬로 깔리는 1스킬. 5인 사이클 안 자리는 아님(MD 축에 가까움).
- [[스킬-스코프]] — user-level(`~/.claude/skills/`)에 깔면 모든 PC 세션에서 자동 발동. 더 강한 baseline 원하면 `~/.claude/CLAUDE.md`로 격상.
