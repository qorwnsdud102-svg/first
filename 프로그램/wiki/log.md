...
## [2026-05-29] lint round 3 | 자체 점검에서 발견한 잔여 정리 → 핵심-맥락·[[엠타트업/2025-10_엠군_부자를훔치다]]·[[엠타트업/2025-10_엠군_M군스토리-자본주의적사고]] (3갱신)
...
## [2026-08-19] ingest | competitor-finder surge_trend 실작업(카테고리 3종 전수) → [[네이버-급등-신규브랜드-탐지-노하우]]·[[ItemScout-API]]·[[네이버-데이터랩-쇼핑인사이트-API]]·[[NAVER-API-HUB]]·[[API-쿼터-경제학]] 5신 · [[측정-지표-함정]]·[[competitor-finder]] 2갱신 (5신, 2갱신)

  - 소스 3종 분업(ItemScout 전수 + 데이터랩 시간축), **데이터랩 빈 달 생략 함정**(신규 0건 거짓 결과), 429 기반 키 로테이션, 브랜드 묶음(접두어 클러스터), 상품수 필터의 카테고리별 상반 효과.
  - 개발자센터→API HUB 이관(7/31 신규발급 차단, 2027-06-30 기존키 종료) 시간표와 이관 3줄 어댑터 기록.
